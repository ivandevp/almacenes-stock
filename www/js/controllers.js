	...
	angular.module('triathlon.controllers', [])
		.controller('UserController', ['$scope', '$http', '$location', function($scope, $http, $location) {
			...
			$scope.login = function() {
				...
				console.log("login!");
				var pin = $scope.pin;
				if (pin.toUpperCase() === "SUPPORT-ADMIN") {
					$scope.processing = false;
					$location.url('/config');
				} else {
					...
					if (general_ip == null || local_ip == null || store_no == null) {
						...
						$http.get("http://" + local_ip + "/" + api_path + "/api/employee/" + pin)
		...
		.controller('StoreController', ['$scope', '$http', function($scope, $http) {
			...
			var general_ip = window.localStorage.getItem("general_ip");
			var api_path = window.localStorage.getItem("api_path");
			$http.get("http://" + general_ip + "/" + api_path + "/api/store")
				.success(function(data, status, headers, config) {
					$scope.stores = data;
				})
				.error(function(data, status, headers, config) {
		...
		.controller('SKUController', ['$scope', '$http', '$location', '$routeParams', function($scope, $http, $location, $routeParams) {
			...
			var general_ip = window.localStorage.getItem("general_ip");
			var api_path = window.localStorage.getItem("api_path");
			$http.get("http://" + general_ip + "/" + api_path + "/api/product/" + sku)
				.success(function(data, status, headers, config) {
					$scope.processing = false;
					$scope.stores = data;					
				})
		...
		.controller('ConfigController', ['$scope', function($scope) {
			...
			$scope.saveSettings = function() {
				...
				if (form_general_ip == null || form_local_ip == null || form_store_no == null || form_api_path == null) {
					...
					window.localStorage.setItem("api_path", $scope.api_path);
